...
>(function SidebarProvider(
  ...
  const isMobile = useBreakpoint("md", { direction: "down" })
...
>(function Sidebar(
  ...
  return (
    <div
      className="group peer hidden text-sidebar-foreground md:block ltr:data-[side=right]:order-last rtl:data-[side=left]:order-last"
      data-state={state}
...
>(function SidebarRail({ className, ...props }, ref) {
  ...
  return (
    <button
      ...
      className={cn(
        "absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-all ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] after:-translate-x-1/2 hover:after:bg-sidebar-border sm:flex",
...
>(function SidebarMenuSkeleton({ className, showIcon = false, ...props }, ref) {
  ...
  // Derive a varied but hydration-stable width from React's deterministic ID.
  const width = `${
    50 +
    Array.from(id).reduce(
      (hash, character) => (hash * 31 + character.charCodeAt(0)) % 40,
      0
    )
  }%`
